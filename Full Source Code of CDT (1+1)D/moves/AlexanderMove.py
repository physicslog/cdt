...
class AlexanderMove(AbstractMove):
	...
	def extract_all_info_and_update(self):
		...
		for i in old_timelike_edges:
			for j in new_timelike_edges:
				for k in new_spacelike_edges:
					# When I found this trick, I loved it!
					tri_vertices = i.vertices | j.vertices | k.vertices
					if len(tri_vertices) == 3: 
						nt = tts_triangle(tri_vertices, [k.id], [i.id, j.id])
						tts_triangle.add(nt)
		...
		# Return the newly created central vertex n, i.e. central vertex instances
		# Because it's very helpful if the metropolis reject the move
		return new_central_vertex
